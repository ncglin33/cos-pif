...
function renderTable(users) {
    ...
    users.forEach(user => {
        ...
        tr.innerHTML = `<td>${user.company || ''}</td><td>${user.name || ''}</td><td>${user.email}</td><td>${formatDate(user.registrationDate)}</td><td>${formatDate(user.expires)}</td><td><span class="status-badge ${status}">${translateStatus(status)}</span></td><td id="actions-${user.id}"></td>`;
